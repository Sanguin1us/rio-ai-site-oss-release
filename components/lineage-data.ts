...
export const RIO_1_NODES: LineageNode[] = [
    { id: 'qwen-2.5-instruct', label: 'Qwen 2.5 Instruct', x: 1, y: 3.5, parents: [], model: undefined },
    { id: 'rio-1', label: 'Rio 1', x: 3, y: 3.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio 1'), icon: Brain },
];
...
export const RIO_1_5_NODES: LineageNode[] = [
    // Origins
    { id: 'qwen-2.5-instruct', label: 'Qwen 2.5 Instruct', x: 0, y: 2, parents: [], model: undefined },
    { id: 'qwen-2.5-vl', label: 'Qwen 2.5 VL', x: 0, y: 5, parents: [], model: undefined, icon: Eye },

    // Primary
    { id: 'rio-1.5', label: 'Rio 1.5', x: 2, y: 2, parents: ['qwen-2.5-instruct'], model: findModel('Rio 1.5'), icon: Brain },

    // Secondary / Experimental
    { id: 'rio-1.6', label: 'Rio 1.6', x: 4, y: 2, parents: ['rio-1.5'], model: findModel('Rio 1.6'), icon: Brain },
    { id: 'ipiranga', label: 'Ipiranga', x: 4, y: 4, parents: ['qwen-2.5-vl', 'rio-1.5'], model: findModel('Ipiranga'), icon: Eye },
    { id: 'niemeyer', label: 'Niemeyer', x: 4, y: 6, parents: ['qwen-2.5-vl', 'rio-1.5'], model: findModel('Niemeyer'), icon: Palette },
];
...
export const RIO_2_NODES: LineageNode[] = [
    ...
    // Gen 1
    { id: 'rio-preview', label: 'Rio Preview', x: 1, y: 4.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio 2.0 Preview') },
    { id: 'rio-fala', label: 'Rio Fala', x: 1, y: 0.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio Fala'), icon: Mic },
    { id: 'rio-transcricao', label: 'Rio Transcrição', x: 1, y: 1.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio 2.0 Transcrição') },
    // Direct connection to Thinking/Tool (skipping Nothink as requested)
    { id: 'rio-thinking', label: 'Rio Thinking', x: 1, y: 2.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio Thinking') },
    { id: 'rio-tool-use', label: 'Rio Tool Use', x: 1, y: 3.5, parents: ['qwen-2.5-instruct'], model: findModel('Rio Tool Use') },
    // Visão
    { id: 'rio-visao', label: 'Rio Visão', x: 1, y: 6, parents: ['qwen-2.5-vl'], model: findModel('Rio 2.0 Visão') },

    // Gen 2 (from Gen 1)
    { id: 'rio-voz', label: 'Rio Voz', x: 2, y: 1, parents: ['rio-fala', 'rio-transcricao'], model: findModel('Rio 2.0 Voz') },

    // Gen 2 (Visão Derivatives)
    { id: 'rio-video', label: 'Rio Vídeo', x: 2, y: 4, parents: ['rio-visao'], model: findModel('Rio Vídeo') },
    { id: 'rio-grounding', label: 'Rio Grounding', x: 2, y: 8, parents: ['rio-visao'], model: findModel('Rio 2.0 Grounding') },
    { id: 'rio-ocr', label: 'Rio OCR', x: 2, y: 6, parents: ['rio-visao'], model: findModel('Rio OCR') },
    { id: 'rio-segmentacao', label: 'Rio Segmentação', x: 2, y: 7, parents: ['rio-visao'], model: findModel('Rio Segmentação') },
    { id: 'rio-visao-dinamica', label: 'Rio Visão Dinâmica', x: 2, y: 5, parents: ['rio-visao'], model: findModel('Rio Visão Dinâmica') },

    // Gen 3 (Integrated)
    { id: 'rio-tool-thinking', label: 'Rio Tool Thinking', x: 2, y: 3, parents: ['rio-thinking', 'rio-tool-use'], model: findModel('Rio Tool Thinking') },
    { id: 'rio-explorer', label: 'Rio Explorer', x: 3, y: 8, parents: ['rio-grounding'], model: findModel('Rio Explorer') },
    ...
    { id: 'rio-search', label: 'Rio Search', x: 3, y: 3, parents: ['rio-tool-thinking'], model: findModel('Rio 2.0 Search') },
    { id: 'rio-detetive', label: 'Rio Detetive', x: 3, y: 4, parents: ['rio-tool-thinking', 'rio-video'], model: findModel('Rio Detetive') },
